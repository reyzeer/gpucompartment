...
int main() {
	...
	long unsigned int checkNumber = 0;
	printf("Check number:\n >");
	scanf("%lu", &checkNumber);
	...
	status |= clGetEventProfilingInfo(kernelEvent, CL_PROFILING_COMMAND_START, sizeof(cl_ulong), &ev_start_time, NULL);
	status |= clGetEventProfilingInfo(kernelEvent, CL_PROFILING_COMMAND_END, sizeof(cl_ulong), &ev_end_time, NULL);
